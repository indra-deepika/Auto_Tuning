...
class GPDataSet:
    ...
    def printdata(self):
        ...
        # Overwrite the CSV file every 50 rounds
        if self.num_previousamples % 50 == 0:
            data = []
            for i in range(self.num_previousamples):
                # Create a dictionary for each sample
                entry = {'Round': i}
                # Add each metric to the dictionary
                for j, metric in enumerate(self.metric_labels):
                    entry[metric] = self.previous_metric_set[i][j]

                for k, knob in enumerate(self.knob_labels):
                    entry[knob] = self.previous_knob_set[i][k]

                data.append(entry)

            df = pd.DataFrame(data)
            df.to_csv('data_every_100_rounds.csv', index=False)
        for i in range(self.num_previousamples):
